function [Emedio,E2medio, nkmed]=metropolisFermioes(T,nequi,nmedidas,N,nmax)
...
nkmed=nk; % matriz para calculo do valor medio da matriz nk
   ...
   [eks,iks]=sort(ek);
   nk(iks(1:N))=1;
   E=sum(eks(1:N)); %  energia total inicial   
      
   estado_particula(1:N)=iks(1:N); %inicialmente todas as particulas estao no estado 1

 npassos=nmedidas+nequi; % numero total de passos
...
for t=1:npassos % ciclo do numero de passos
    for iter=1:N % itreacoes sobre particulas - para atualizar o estado de uma particula uma vez em cada passo 
        ...
        nx=mod(ik-1,nmax)+1;ny=floor(ik/nmax)+1;
         ...
         nxv=mod(ikv-1,nmax)+1;nyv=floor(ikv/nmax)+1;
    
        % calcula variaçao de energia
            dE=nxv^2+nyv^2-(nx^2+ny^2);
            if (rand(1) <= min(1,nv(ik)*(nk(ikv)+1)/(nv(ikv)*nk(ik))*exp(-dE/T)) && nk(ikv)==0 ); 
